...
class Camera:
    ...
    def sample_rays(self, n_sample_ray=4096, precrop_frac=1.0):
        ...
        if precrop_frac < 1.0:
            ...
            coords = torch.stack(
                torch.meshgrid(
                    torch.linspace(0, self.img_h - 1, self.img_h),
                    torch.linspace(0, self.img_w - 1, self.img_w)
                ), -1)
    ...
    #     """
    #     :param u: (N,) torch.Tensor.
    #     :param v: (N,) torch.Tensor.
    #     :param depth: (N,) torch.Tensor.
    #     :return:
    #         pc_world: (N, 3) torch.Tensor.
    #     """
    #     u = (u - self.K[0, 2]) / self.K[0, 0]
    #     v = - (v - self.K[1, 2]) / self.K[1, 1]
    #     uvd = torch.stack((u, v, -torch.ones_like(u)), 1)
    #     pc_cam = uvd * depth.unsqueeze(1)
    #
    #     cam_rot, cam_transl = self.pose[:3, :3], self.pose[:3, 3]
    #     pc_world = torch.einsum('ij,nj->ni', cam_rot, pc_cam) + cam_transl
    #     return pc_world